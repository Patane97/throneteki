...
class DetermineDominance extends BaseStep {
    ...
    continue() {
        this.game.raiseEvent('onDominanceDetermined', {}, (event) => {
            ...
            if (result.totalTied) {
                ...
                if (choosingPlayer) {
                    ...
                    const prompt = new ChoosePlayerPrompt(this.game, choosingPlayer, {
                        condition: (player) =>
                            potentialWinners.map((pw) => pw.player).includes(player),
    ...
    applyResult(event, result) {
        event.winner = result.player;
        event.difference = result.difference;
        event.chosenBy = result.chosenBy;
        ...
        if (event.winner) {
            ...
            if (action.allow() && !event.winner.hasFlag(Flags.player.cannotGainDominancePower)) {
                if (event.chosenBy) {
                    this.game.addMessage(
